...
public class ME_CustomLight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Shader.SetGlobalInt("ME_LightCount", lightCount);
        Shader.SetGlobalVectorArray("ME_LightPositions", ListToArrayWithMaxCount(lightPositions, MaxLightsCount));
        Shader.SetGlobalVectorArray("ME_LightColors", ListToArrayWithMaxCount(lightColors, MaxLightsCount));

        var ambientColor = SampleLightProbesUp(transform.position, 0.5f);
        Shader.SetGlobalColor("ME_AmbientColor", ambientColor);
    }

    void OnDisable()
    {
        Shader.SetGlobalInt("ME_LightCount", 0);
        Shader.SetGlobalVectorArray("ME_LightPositions", new[] { Vector4.zero });
        Shader.SetGlobalVectorArray("ME_LightColors", new[] { Vector4.zero });
